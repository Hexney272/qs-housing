...
local function BlackMoneyStorage()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title = i18n.t('esx_stash.title'),
            align = 'right',
            elements = {
                { label = i18n.t('esx_stash.store'),    value = 's' },
                { label = i18n.t('esx_stash.withdraw'), value = 'w' }
            },
        },
        function(data, menu)
            if data.current.value == 's' then
                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'putAmount', { title = i18n.t('esx_stash.black_money_amount') }, function(data3, menu3)
                    local amount = tonumber(data3.value)

                    if amount == nil then
                        Notification(i18n.t('esx_stash.invalid_amount'), 'error')
                    else
                        if amount >= 0 then
                            TriggerServerCallback('qb-houses:server:depositBlackMoney', function(success)
                                if success then
                                    Notification(i18n.t('esx_stash.black_money_success'), 'success')
                                else
                                    Notification(i18n.t('esx_stash.cant_afford'), 'error')
                                end
                            end, amount)
                            menu3.close()
                        else
            ...
            elseif data.current.value == 'w' then
                TriggerServerCallback('qb-houses:server:getBlackMoney', function(count)
                    ...
                    table.insert(elements, { label = i18n.t('esx_stash.black_money_name', { count = count }), value = 'black_money' })

                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'withdrawItem', {
                        title = i18n.t('esx_stash.withdraw'),
                        align = 'right',
                        elements = elements
                            ...
                            if amount == nil then
                                ...
                                if amount >= 0 then
                                    TriggerServerCallback('qb-houses:server:withdrawBlackMoney', function(success)
                                        if success then
                                            Notification(i18n.t('esx_stash.black_money_success_withdraw'), 'success')
                                        else
                                            Notification(i18n.t('esx_stash.cant_afford'), 'error')
                                        end
                                    end, amount)
        ...
        function(data, menu)
            if data.current.value == 's' then
                TriggerServerCallback('qb-houses:server:getInventory', function(inv)
                    ...
                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'storeItem', {
                        title = i18n.t('esx_stash.player_inventory'),
                        align = 'right',
                        elements = elements
                    }, function(data2, menu2)
            ...
            elseif data.current.value == 'w' then
                TriggerServerCallback('qb-houses:server:getHouseInventory', function(inv)
                    ...
                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'withdrawItem', {
                        title = i18n.t('esx_stash.house_inventory'),
                        align = 'right',
                        elements = elements
                    }, function(data2, menu2)
                        ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'putAmount', { title = 'Amount' }, function(data3, menu3)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'storage',
        {
            ...
            elements = {
                ...
                { label = i18n.t('esx_stash.items'),       value = 'i' },
                { label = i18n.t('esx_stash.weapons'),     value = 'w' },
                { label = i18n.t('esx_stash.black_money'), value = 'b' }
            },
        },
        function(data, menu)
