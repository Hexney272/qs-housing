...
-- Auxiliary function to know if it is administrator
function PlayerIsAdmin(source)
    if not Config.AllowAdminsToCreateHouses then return false end
    local identifier = GetIdentifier(source)
    return Admins[identifier] == true
end

-- Event to create a house (example)
RegisterNetEvent('houses:createHouse', function(coords)
    local src = source
    local identifier = GetIdentifier(src)

    if not Admins[identifier] then
        print('[HOUSES] UNAUTHORIZED attempt to create a house of: ' .. (identifier or 'unknown'))
        return
    end

    -- Here you must save the data in your home system or database
    print(('[HOUSES] House created by %s at coordinates x=%.2f y=%.2f z=%.2f'):format(identifier, coords.x, coords.y, coords.z))
end)

-- Framework stub functions
function RegisterUsableItem(name, cb)
    ImplementError('RegisterUsableItem is not supported with standalone')
    return false
end

function GetPlayerFromId(source) return { source = source, identifier = GetIdentifier(source) } end

function GetPlayerSource(player) return player.source end
...
function GetPlayerFromIdentifier(identifier)
    ...
    for _, v in pairs(players) do
        if GetIdentifier(v) == identifier then
            return { source = v, identifier = identifier }
        end
    end
    return nil
end
...
function UpdateInside(src, insideId, bool) ImplementError('Update Inside : You need to implement this function for your framework.') end
